...
const {  setSharedScreenUserId, sharedScreenUserId} = useCallState();
  ...
  return (
    ...
    <video autoPlay muted playsInline id={`video-${participant?.user_id}`} ref={videoRef} style={{
...
export const Video = ({ participants }) => {
  ...
  let participant;
  // useEffect(() => {
    participant = participants.filter(userList => userList?.tracks?.screenVideo?.state === "playable");
  ...
  return (
    <>
      {/* {participants.map((p) => ( */}
       <VideoItem participant={participant?.length? participant[0]:{}} key={`p-${participant?.length? participant[0]?.user_id : ''}`} />
      {/* ))} */}
    </>
  );
};
